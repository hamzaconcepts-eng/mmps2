...
export default function ClassEditForm({
  ...
  const [form, setForm] = useState({
    ...
    class_supervisor_id: cls.class_supervisor_id || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        router.push(`/${locale}/classes/${cls.id}`);
        router.refresh();
      }, 800);
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        <Button type="button" variant="glass" size="sm" onClick={() => router.push(`/${locale}/classes/${cls.id}`)} disabled={loading}>
          {labels.cancel}
        </Button>

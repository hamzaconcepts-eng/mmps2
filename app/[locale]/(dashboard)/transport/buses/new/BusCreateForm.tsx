...
export default function BusCreateForm({ areas, locale, defaultAreaId, labels }: BusCreateFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    plate_number: '',
    capacity: '40',
    transport_area_id: defaultAreaId || '',
    driver_name: '',
    driver_name_ar: '',
    driver_father_name: '',
    driver_father_name_ar: '',
    driver_family_name: '',
    driver_family_name_ar: '',
    driver_phone: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setTimeout(() => { router.push(`/${locale}/transport/buses/${result.busId}`); router.refresh(); }, 800);
    } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Input label={labels.busNumber} value={form.bus_number} onChange={(e) => handleChange('bus_number', e.target.value)} required disabled={loading} locale={locale} placeholder="BUS-01" />
